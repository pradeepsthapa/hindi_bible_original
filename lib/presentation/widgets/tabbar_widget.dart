...
//                     final List list = widget.biblebook[ref.watch(bookIndexProvider.state).state].chapter;
//                     final List<ChapterElement> chapters = list.map((e) => ChapterElement.fromJson(e)).toList();
//                     return Wrap(
//                       children: chapters.map((e) {
//                         return TextButton(onPressed: (){
//                           ref.read(boxStorageProvider).saveChapterIndex(e.cnumber??'1');
//                           ref.read(pageController).animateToPage(int.parse(e.cnumber??'1')-1,duration: Duration(milliseconds: 400),curve: Curves.decelerate);
...
//                         child: Wrap(
//                           children: chapters[int.parse(ref.watch(chapterNumberProvider.state).state)-1].vers!.map((e) {
//                             return GestureDetector(
//                               onTap: (){
//                                 ref.read(scrollController.state).state.scrollTo(index: chapters[int.parse(ref.watch(chapterNumberProvider.state).state)-1].vers!.indexOf(e), duration: Duration(milliseconds: 400),curve: Curves.decelerate);
